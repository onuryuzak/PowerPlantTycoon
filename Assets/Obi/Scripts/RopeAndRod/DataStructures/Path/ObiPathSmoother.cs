...
namespace Obi
{
    ...
    public class ObiPathSmoother : MonoBehaviour
    {
        ...
        [Range(0, 1)]
        [Tooltip(
            "Curvature threshold below which the path will be decimated. A value of 0 won't apply any decimation. As you increase the value, decimation will become more aggresive.")]
        public float decimation = 0;

        [Range(0, 3)]
        [Tooltip(
            "Smoothing iterations applied to the path. A smoothing value of 0 won't perform any smoothing at all. Note that smoothing is applied after decimation.")]
        ...
        public ObiList<ObiList<ObiPathFrame>> rawChunks = new ObiList<ObiList<ObiPathFrame>>();
        public ObiList<ObiList<ObiPathFrame>> smoothChunks = new ObiList<ObiList<ObiPathFrame>>();
        ...
        private void PathFrameFromParticle(ObiRopeBase actor, ref ObiPathFrame frame, int particleIndex,
            bool interpolateOrientation = true)
        {
            ...
            if (actor.usesOrientedParticles)
            {
                ...
                Quaternion average = w2lRotation *
                                     (interpolateOrientation
                                         ? Quaternion.SlerpUnclamped(current, previous, 0.5f)
                                         : current);
                ...
                for (int i = 0; i < rawChunks.Count; ++i)
                {
                    ...
                    for (int m = 1; m <= rawChunks[i].Count; ++m)
                    {
                        ...
                        else
                        {
                            ...
                            frame_1.tangent =
                                ((frame_1.position - frame_2.position) + (frame_0.position - frame_1.position))
                                .normalized;
        ...
        private bool Decimate(ObiList<ObiPathFrame> input, ObiList<ObiPathFrame> output, float threshold)
        {
            ...
            while (stack.Count > 0)
            {
                ...
                for (int i = index + 1; i < range.y; ++i)
                {
                    ...
                    {
                        float d = Vector3.SqrMagnitude(ObiUtils.ProjectPointLine(input[i].position,
                            input[range.x].position, input[range.y].position, out mu) - input[i].position);
        ...
        */
        private void Chaikin(ObiList<ObiPathFrame> input, ObiList<ObiPathFrame> output, uint k)
        {
            ...
            output[pCount * n0 - pCount + 1] = (0.5f - twoRaisedToMinusKPlus1) * input[n0 - 1] +
                                               (0.5f + twoRaisedToMinusKPlus1) * input[n0];
